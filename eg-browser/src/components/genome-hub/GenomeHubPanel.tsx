...
export default function GenomeHubPanel() {
    ...
    const { hasGroups, groupedGenomes } = useMemo(() => {
        const hasGroups = customGenomes.some(genome => genome.group !== undefined);
    ...
    const sortedGroups = useMemo<SortedGroupEntry[]>(() => {
        ...
        return Object.entries(groupedGenomes)
        ...
        >
            <div className="flex flex-row justify-between items-center">
                <div className="flex flex-col gap-2">
                    ...
                    <p className="text-sm">
                        Chromosomes: {genome.chromosomes.length > 0 ?
                            genome.chromosomes.slice(0, 3).map(chr => chr.name).join(", ") +
                            (genome.chromosomes.length > 3 ? ` +${genome.chromosomes.length - 3} more` : "")
                            : "None"}
                    </p>
                </div>
                {loading ? (
