  ...
  React.useEffect(() => {
    ...
    if (prevActive.current !== active) {
      ...
      if (mergedStyle.backgroundColor)
        el.style.backgroundColor = mergedStyle.backgroundColor as string;
  ...
  const handleMouseEnter = () => {
    ...
    if (el && bg) {
      if (!originalBg.current)
        originalBg.current = el.style.backgroundColor || bg;
  ...
  return (
    <button
      ...
      className={classNames(
        "w-20 h-9 rounded-xs flex items-center justify-center",
