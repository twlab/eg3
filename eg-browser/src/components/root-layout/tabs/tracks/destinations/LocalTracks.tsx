...
function AddLocalTracks() {
    ...
    const [trackState, setTrackState] = React.useState<LocalTrackState>({
        type: "bigWig",
        assembly: session?.genome ?? "hg19",
        files: null,
        indexSuffix: ".tbi",
        msg: "",
    });

    const [selectedStep, setSelectedStep] = React.useState<AddLocalTracksStep | null>(AddLocalTracksStep.TRACK_TYPE);
    ...
    const handleSubmit = () => {
        ...
        const needsIndex = !ONE_TRACK_FILE_LIST.includes(type.toLowerCase());

        if (needsIndex && fileList.length !== 2) {
            setTrackState(prev => ({ ...prev, msg: "Please select both track and index files" }));
            return;
        }
            ...
            <StepAccordion<AddLocalTracksStep>
                ...
                items={[
                    ...
                    },
                    {
                        ...
                        valuePreview: trackState.files ? `${trackState.files.length} file(s)` : undefined,
                        component: <TrackFileUpload
                    ...
                    },
                    {
                        ...
                        valuePreview: trackState.assembly ? `${trackState.assembly}` : undefined,
                        component: <TrackAssembly
        ...
        <div className="space-y-4 py-4">
            <div>
                ...
                <input
                    type="file"
                    className="w-full p-2 border rounded"
                    multiple={ONE_TRACK_FILE_LIST.includes(type.toLowerCase())}
...
function AddLocalHub() {
    ...
    return (
        <div className="space-y-8 py-4">
            <div>
                <h3 className="text-lg font-semibold mb-4">Choose a folder containing hub.config.json:</h3>
                <input
                    type="file"
                    className="w-full p-2 border rounded"
            ...
            <div>
                <h3 className="text-lg font-semibold mb-4">Or choose multiple files (including hub.config.json):</h3>
                <input
                    type="file"
                    className="w-full p-2 border rounded"
                    multiple
