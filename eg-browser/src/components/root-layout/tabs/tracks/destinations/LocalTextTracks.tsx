...
export default function LocalTextTracks() {
    ...
    const [selectedStep, setSelectedStep] = React.useState<AddTextTrackStep | null>(AddTextTrackStep.TRACK_TYPE);
    ...
    const handleFileUpload = async (files: FileList | null) => {
        ...
        const tracks: ITrackModel[] = fileList.map(
            (file) => ({
                type: typedArray[0],
                url: "",
                fileObj: file,
                ...
                items={[
                    {
                        ...
                        valuePreview: trackState.textType ? TEXT_TYPE_DESC[trackState.textType as keyof typeof TEXT_TYPE_DESC].label : undefined,
                        component: <SelectTextType
...
function SelectTextType({ selectedType, onTypeChange }: SelectTextTypeProps) {
    const currentType = TEXT_TYPE_DESC[selectedType as keyof typeof TEXT_TYPE_DESC];
        ...
        <div className="space-y-4 py-4">
            ...
            <div className="mt-4">
                ...
                <div className="mt-4">
                    ...
                    <pre className="font-mono text-sm bg-[#E8E8E8] p-4 rounded whitespace-pre overflow-x-auto">{currentType.example}</pre>
                </div>
            </div>
        </div>
    );
...
function ConfigureTrack({ onOptionsChange, onHugeFileToggle, isFileHuge }: ConfigureTrackProps) {
    return (
        <div className="space-y-4 py-4">
            <div>
            ...
            <div className="flex items-center space-x-2">
                ...
                <label htmlFor="hugeCheck" className="text-sm">
                    Use a Worker thread <span className="text-gray-500">(Check if your file is huge.)</span>
                </label>
            </div>
        </div>
    );
