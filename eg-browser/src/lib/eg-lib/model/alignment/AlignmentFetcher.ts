...
import { fetchGenomeAlignWorker } from "../../dataSources/WorkerTSHook";
...
export class AlignmentFetcher {
  ...
  constructor(
    public primaryGenomeConfig: GenomeConfig,
    ...
    this.queryGenome =
      queryTrack.querygenome || queryTrack.getMetadata("genome");
    ...
    if (!url) {
      ...
      const theTrack =
        comparisonTracks.find(
          (track: any) => track.querygenome === this.queryGenome
        ) || {};
      url = theTrack.url;
    }
    if (!url) {
  ...
  async fetchAlignment(
    ...
    if (this.isBigChain) {
      const rawRecords: any[] = await this._dataSource.getData(
        fetchRegion,
        basesPerPixel,
        {}
      );
