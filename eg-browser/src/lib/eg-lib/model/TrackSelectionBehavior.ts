...
class TrackSelectionBehavior {
  ...
  handleMetadataClick(
    ...
    while (
      ...
      tracks[maxIndex].getMetadata(term) === termValue
    ) {
      maxIndex++;
    }
    ...
    if (this.isToggleEvent(event)) {
      const nextSelections = tracks.map((track) => track.isSelected);
      const isAlreadyAllSelected = tracks
        .slice(minIndex, maxIndex)
        .every((track) => track.isSelected);
      if (isAlreadyAllSelected) {
        // All selected?  Deselect the block.
        _.fill(nextSelections, false, minIndex, maxIndex);
      } else {
        // Some, or none selected?  Select the entire block.
        _.fill(nextSelections, true, minIndex, maxIndex);
      }
      return nextSelections;
    } else {
      // Event not a toggle-selection one: select the block and deselect all others
      const nextSelections = Array(tracks.length).fill(false);
      _.fill(nextSelections, true, minIndex, maxIndex);
      return nextSelections;
    }
