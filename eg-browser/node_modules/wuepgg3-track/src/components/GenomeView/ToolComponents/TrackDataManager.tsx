  ...
  > {
    ...
    componentDidUpdate(prevProps: DataManagerProps) {
      if (this.props.viewRegion !== prevProps.viewRegion) {
        this.fetchAllTracks((trackConfig) =>
          trackConfig.shouldFetchBecauseRegionChange(trackConfig.getOptions())
        );
      } else if (this.props.tracks !== prevProps.tracks) {
        this.detectChangedTracks(prevProps.tracks); // Fetch some
      } else if (
        this.props.viewAndAlignmentPromise !== prevProps.viewAndAlignmentPromise
      ) {
    ...
    async fetchTrack(track: TrackModel) {
      ...
      const { primaryView, alignments } =
        await this.props.viewAndAlignmentPromise;
      ...
      let visRegion,
        alignment = null;
      if (!genome || genome === this.props.genome) {
        // Is primary genome?
      ...
      const dataRegion = options.fetchViewWindowOnly
        ? primaryView.viewWindowRegion
        : visRegion;
      ...
      try {
        ...
        if (this.isViewStillFresh(requestedRegion)) {
          this.dispatchTrackUpdate(track, {
            ...
            meta: dataSource.getCurrentMeta(
              dataRegion,
              this.props.basesPerPixel,
              options
            ),
