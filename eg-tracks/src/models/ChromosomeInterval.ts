...
class ChromosomeInterval extends OpenInterval implements IChromosomeInterval {
  ...
  static mergeAdvanced<T>(
    ...
    for (const chrName in groupedByChromosome) {
      ...
      objectsForChromosome.sort(
        (a, b) => iteratee(a).start - iteratee(b).start
      );
   ...
   */
  static mergeOverlaps(
    intervals: ChromosomeInterval[],
    mergeDistance = 2000
  ): ChromosomeInterval[] {
    const mergeInfos = ChromosomeInterval.mergeAdvanced(
      intervals,
      mergeDistance,
      _.identity
    );
  ...
  *[Symbol.iterator]() {
  ...
  getOverlap(other: ChromosomeInterval): ChromosomeInterval | null {
    if (this.chr !== other.chr) {
      ...
      return overlap
        ? new ChromosomeInterval(this.chr, overlap.start, overlap.end)
        : null;
    }
  }
