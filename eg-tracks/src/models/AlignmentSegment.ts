...
export class AlignmentSegment extends FeatureSegment {
  ...
  static fromFeatureSegment(segment: FeatureSegment): AlignmentSegment {
    return new AlignmentSegment(
      segment.feature as AlignmentRecord,
  ...
  getQueryLocus() {
    ...
    return new ChromosomeInterval(
      queryLocus.chr,
      queryStrand === "+"
        ? Math.min(queryLocus.start + this.relativeStart, queryLocus.end)
  ...
  getQueryLocusFine() {
    ...
    const baseOffset = countBases(
      querySeq.substring(0, this.sequenceInterval.start)
    );
