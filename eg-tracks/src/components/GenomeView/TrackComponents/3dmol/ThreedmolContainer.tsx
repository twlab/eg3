...
interface ComponentState {
  ...
  legendMinColor: any;
  legendMaxColor: any;
  colorScale: any; // Replace 'any' with a more specific type if known
  chrom: string;
  start: number;
  end: number;
  thumbStyle: string;
  hoveringAtom: any; // Replace 'any' with a more specific type if known
  hoveringX: number;
  hoveringY: number;
  paintMethod: string;
  paintRegion: string;
  ...
  annotationFileObject: any; // Replace 'any' with a more specific type if known
  numFileObject: any; // Replace 'any' with a more specific type if known
  newG3dUrl: string;
  animateMode: boolean;
  frameAtoms: any[]; // Replace 'any' with a more specific type if known
  frameLabels: any[]; // Replace 'any' with a more specific type if known
  currentFrame: number;
  myShapes: Record<string, any>; // Replace 'any' with a more specific type if known
  myShapeLabel: string;
  ...
  myArrows: Record<string, any>; // Replace 'any' with a more specific type if known
  labelStyle: string;
  annoFormat: string;
  ...
  staticCategories: any; // Replace 'any' with a more specific type if known
  numFormat: string;
  envelopCenter: any; // Replace 'any' with a more specific type if known
...
> {
  ...
  async componentDidUpdate(prevProps, prevState) {
    ...
    if (
      legendMaxColor !== prevState.legendMaxColor ||
      legendMinColor !== prevState.legendMinColor
    ) {
    ...
    if (
      envelopColor !== prevState.envelopColor ||
      envelopOpacity !== prevState.envelopOpacity
    ) {
      this.updateEnvelop();
    }
    if (
  ...
  drawImageLabel = (displayConfig) => {
    ...
    if (imageInfo) {
      ...
      const atoms: Array<any> = findAtomsWithRegion(
  ...
  drawMyShapes = (displayConfig) => {
    ...
    if (!this.shapes.length) {
      ...
      this.setState({
        message:
          "cannot find matched atoms to label or no model is displaying, skip",
      });
      return;
    }
    this.viewer.render();
  ...
  prepareAtomKeeper = () => {
    ...
    const resString = resolution.toString();
    if (
      _.isEmpty(this.atomKeeper) ||
      !this.atomKeeper.hasOwnProperty(resString)
    ) {
  ...
  drawAnchors3d = (displayConfig) => {
    ...
    if (!this.arrows.length) {
      ...
      this.setState({
        message:
          "cannot find matched atoms to point or no model is displaying, skip",
      });
      return;
    }
    this.viewer.render();
    ...
    this.setState(
      {
        ...
        menuFlexDirection:
          value === "right" || value === "left" ? "column" : "row",
        childShow: false,
      },
      () => {
        this.setState({
  ...
  setAtomClickable = (at) => {
    ...
    this.setState({
      hoveringAtom: at,
      hoveringX: screenXY.x,
      hoveringY: screenXY.y,
    });
  };
  ...
  prepareAtomData = async () => {
    ...
    const resString = resolution.toString();
    const stateAtoms: Array<any> = [],
      stateLabels: Array<any> = [];
    let atoms2: any; //, atoms; // atoms2 original object, atoms with added events callback
    if (this.atomData.hasOwnProperty(resString)) {
      ...
      Object.keys(atoms2).forEach((hap) => {
        ...
        atoms2[hap].forEach((atom) => {
          if (
            !this.atomStartsByChrom[resString][hap].hasOwnProperty(atom.chain)
          ) {
            this.atomStartsByChrom[resString][hap][atom.chain] = [
              atom.properties.start,
            ];
          } else {
    ...
    const envelopRadius =
      (2 / 3) *
      Math.min(envelopCenter.distanceTo(p1), envelopCenter.distanceTo(p2));
    ...
    this.setState({
      message: "",
      frameAtoms: stateAtoms,
      frameLabels: stateLabels,
    });
  };
  ...
  initEnvelop = () => {
    const { envelopRadius, envelopCenter, envelopOpacity, envelopColor } =
      this.state;
    ...
    this.envelop!.setStyle(
      {},
      {
        sphere: {
          radius: envelopRadius,
          color: envelopColor,
  ...
  highlightRegions = (tmpModelDisplayConfig: any = null) => {
    ...
    if (validateRegion) {
      ...
      this.setState({
        message: "cannot find matched region to highlight, skip",
      });
      return;
    }
  };
  ...
  paintWithBigwig: any = async (bwUrl, resolution, regions, chooseRegion) => {
    ...
    if (_.isEmpty(keepers)) {
      this.setState({
        message:
          "bigwig/expression file empty or error parse file, please check your file",
        paintRegion: "none",
      });
      return;
    }
  ...
  paintBigwig = async (chooseRegion) => {
    this.setState({
      paintRegion: chooseRegion,
      message: "numerical painting...",
    });
    const {
      useExistingBigwig,
      bigWigUrl,
    ...
    if (numFormat === "bwtrack") {
      if (!bwUrl.length) {
        this.setState({
          message: "bigwig url for paint is empty, abort...",
          paintRegion: "none",
        });
        return;
      }
  ...
  getCompartmentData = async () => {
    const { compartmentFileUrl, compartmentFileObject, uploadCompartmentFile } =
      this.state;
    if (!compartmentFileUrl.length && !compartmentFileObject) {
      this.setState({ message: "compartment url or file empty, abort..." });
      return;
    }
    const key = uploadCompartmentFile
      ? compartmentFileObject.name
      : compartmentFileUrl;
    ...
    if (!data) {
      this.setState({
        message:
          "file empty or error parse compartment file, please check your file 1",
      });
      return;
    }
    const comp = {};
  ...
  paintAnnotation = async (chooseRegion) => {
    ...
    if (_.isEmpty(anndata)) {
      this.setState({
        message:
          "no system cytoband data, file empty or error parse annotation file, please check your file",
        paintAnnotationRegion: "none",
      });
      return;
    }
  ...
  paintWithAnnotation = (anndata, regions, chooseRegion) => {
    ...
    switch (annoFormat) {
      case "cytoband":
        this.setState({
          staticCategories: CYTOBAND_COLORS_SIMPLE,
          categories: null,
        });
        break;
      case "bedrgb":
      ...
      case "refgene":
        ...
        this.setState({
          categories: { [glabel]: this.state[glabel] },
          staticCategories: null,
        });
        break;
      case "bed4":
      ...
      case "cell2014":
        this.setState({
          categories: { A1, A2, B1, B2, B3, B4, NA },
          staticCategories: null,
        });
        break;
      default:
  ...
  addNewG3D = async (url, key, resolution) => {
    ...
    if (!data) {
      this.setState({
        message: "g3d model file empty or resolution not exist, abort",
      });
      return;
    }
  ...
  prepareModelFrames = async () => {
    ...
    const newatoms: any = await this.addNewG3D(newG3dUrl, key, resolution);
  ...
  syncHic = () => {
    ...
    this.props.onGetViewer3dAndNumFrames({
      viewer3d: this.viewer,
      numFrames: this.viewer.getNumFrames(),
    });
  ...
  addRegionToMyShapes = () => {
    ...
    if (labelStyle === "shape") {
      ...
      try {
        ...
        this.setState({
          message:
            "error parse the region, format should like: chr:start-end, chr start end etc.",
        });
        return;
      }
  ...
  parseUploadedRegionFile = async (fileobj) => {
    const dataString: any = await readFileAsText(fileobj);
