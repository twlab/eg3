...
interface BackgroundedTextProps {
  ...
  x?: number; // x location of the text
  y?: number; // y location of the text
  dominantBaseline?: "hanging" | "middle" | "baseline"; // Vertical alignment of text
  textAnchor?: "start" | "middle" | "end"; // Horizontal alignment of text
  children?: string; // The actual text to display
  // fontSize: the only INVALID prop - use `height` instead
  dy?: string;
  // Special props for this component
  height?: number; // Height of the text - use instead of fontSize
  horizontalPadding?: number; // Horizontal padding of background box
  backgroundColor?: string; // Color of the background. By default, renders no background.
  backgroundOpacity?: number; // Opacity of the background
  italicizeText?: any;
  fontSize?: any;
  fill?: any;
}
class BackgroundedText extends React.Component<BackgroundedTextProps> {
  static propTypes = {
    // Props that are associated with <text>
  ...
  render() {
    ...
    if (fontSize) {
      console.warn(
        "The `fontSize` prop is invalid.  Use the `height` prop to set font size."
      );
    }
    ...
    return (
      <React.Fragment>
        ...
        <text
          {...textProps}
          fontStyle={italicizeText ? "italic" : "normal"}
          fontSize={1.5 * height!}
        >
          {children}
        </text>
