...
export class BamAnnotation extends React.Component<BamAnnotationProps, {}> {
  ...
  renderRead(placedSegment: PlacedSegment) {
    ...
    for (; alignIndex < maxIndex; alignIndex++, x += widthOfOneBase) {
      if (
        alignment.reference.charAt(alignIndex) ===
        alignment.read.charAt(alignIndex)
      ) {
        // Sequence match; do nothing
        continue;
      }

      if (alignment.reference.charAt(alignIndex) === "-") {
        // Insertion: no base in reference
        // TODO
      } else {
        // Base in reference exists: deletion or mismatch
        // color = no info in the read ? deletion : sequence mismatch
        const color =
          alignment.read.charAt(alignIndex) === "-"
            ? options.deletionColor
            : options.mismatchColor;
        elements.push(
          <rect
  ...
  render() {
    ...
    const placedAligned = FEATURE_PLACER.placeFeatureSegments(
      placedRecord,
      segments.aligned
    );
    const placedSkipped = FEATURE_PLACER.placeFeatureSegments(
      placedRecord,
      segments.skipped
    );
    return (
      <TranslatableG y={this.props.y} onClick={this.handleClick}>
