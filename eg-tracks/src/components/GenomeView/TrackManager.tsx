...
import _, { initial, throttle } from "lodash";
        ...
        for (const key in trackFetchedDataCache.current) {
          ...
          if (
            ...
            if (curTrackModel) {
              ...
              tempTrackModel["shouldPlaceRegion"] = initialLoad.current && curTrackCache.usePrimaryNav && !useFineModeNav.current && !curTrackCache.useExpandedLoci

              if (tempTrackModel["shouldPlaceRegion"] && curDataIdx !== curIdx) {
                continue
              }
  ...
  async function createCache(fetchRes: { [key: string]: any }) {
    ...
    if (
      ...
      const formattedData = formatDataByType(
        ...
        fetchRes.trackState.regionLociArr,
      );
      const initialIdx = [fetchRes.trackDataIdx + 1, fetchRes.trackDataIdx, fetchRes.trackDataIdx - 1];
      ...
      if (fetchRes.trackModel.shouldPlaceRegion) {
        ...
        if (!Array.isArray(formattedData) && typeof formattedData === "object") {
          ...
          console.log("HUH2", formattedData)
          for (let i = 0; i < 3; i++) {
