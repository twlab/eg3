...
const TrackManager: React.FC<TrackManagerProps> = memo(function TrackManager({
  ...
  async function fetchGenomeData(initial: number = 0, trackSide, dataIdx) {
    ...
    // AFTER SETTING THE NEW NAV WE GOTTA CACHE THE NAV OF THE VISISTED REGION
    ...
    console.log(globalTrackState.current, "globalTRACKSTATE B4 fetch");
    try {
  ...
  async function createCache(fetchRes: { [key: string]: any }) {
    ...
    if (isInteger(fetchRes.missingIdx)) {
      ...
      cacheFetchedData({
        ...
        trackData: result,
        trackState: {
          side: tmpTrackState.side,
          initial: tmpTrackState.initial,
        },
