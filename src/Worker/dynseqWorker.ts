...
import { scaleLinear } from 'd3-scale';.window.importScripts('path/to/d3-scale.js').bind(window); // Adjust the path as needed
const workerFunction = function () {
  self.onmessage = (event: MessageEvent) => {
    ...
    for (let i = 0; i < trackData.trackGene.length; i++) {
      let startPos = trackData.trackGene[i][1];
      for (let j = 0; j < trackData.trackGene[i][0].length; j++) {
        let singleStrand = trackData.trackGene[i][0][j];
    ...
    for (let i = 0; i < avgPos.length; i++) {
      let scales = computeScales(avgPos[i], avgNeg[i], 0, trackData.bpToPx);
