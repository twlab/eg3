...
class BedSourceWorker extends WorkerRunnableSource {
  ...
  async getDataForLocus(locus) {
    ...
    for (let block of blocks) {
      ...
      const rawData = await requestBinary(this.url, {
        start: startByte,
        end: endByte,
      });
      const uncompressed = unbgzf(rawData);
      const slicedData =
        startOffset > 0 ? uncompressed.slice(startOffset) : uncompressed;
      featuresForEachBlock.push(
        this._parseAndFilterFeatures(slicedData, chr, start, end)
      );
    }
