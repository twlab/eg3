...
interface SelectedRegionBoxProps {
  viewRegion: DisplayedRegionModel; // Entire region being visualized
  width: number; // Draw width of the view region
  selectedRegion: DisplayedRegionModel; // Region that is selected
  /**
   ...
   */
  onNewViewRequested: (newStart: number, newEnd: number) => void; // Function that takes arguments [number, number]
  x?: any;
  y?: any;
}
class SelectedRegionBox extends React.Component<SelectedRegionBoxProps> {
  static propTypes = {
  ...
  gotoPressed(event) {
    ...
    let halfWidth = 0;
    if (
      this.props.selectedRegion.getWidth() < this.props.viewRegion.getWidth()
    ) {
    ...
    this.props.onNewViewRequested(
      regionCenter - halfWidth,
      regionCenter + halfWidth
    );
  }
  ...
  render() {
    ...
    if (xEnd <= 0) {
      ...
      gotoButton = (
        <polygon points={points} style={BOX_STYLE} onClick={this.gotoPressed} />
      );
      gotoText = (
        <text
          x={LABEL_X_PADDING}
