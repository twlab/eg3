...
class Track extends React.Component<TrackProps> {
  ...
  render() {
    const {
      ...
      viewWindow,
      metadataTerms,
      xOffset, // Track container props
      legend,
      visualizer,
      message,
      isLoading,
      error,
      options, // Track subtype props
      style,
      panningAnimation,
      zoomAnimation,
    } = this.props;
...
function ViewWindow(props: ViewWindowProps): JSX.Element {
  ...
  if (props.panningAnimation === "left") {
    ...
    innerStyle.transform = `translateX(${
      -fullWidth + viewWindow.getLength()
    }px)`;
    innerStyle.transition = "transform 1s";
  } else {
  ...
  if (props.zoomAnimation !== 0) {
    innerStyle.transform = `scale(${1 / props.zoomAnimation!}, 1) translateX(${
      -viewWindow.start + xOffset
    }px)`;
    innerStyle.transformOrigin = `${viewWindow.getLength() / 2}px center`;
    innerStyle.transition = "transform 1s";
  }

  return (
    <div style={outerStyle}>
      <div style={innerStyle as any}>{children}</div>
    </div>
  );
}
