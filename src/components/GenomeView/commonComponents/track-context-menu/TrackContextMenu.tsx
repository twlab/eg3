...
class TrackContextMenu extends React.PureComponent {
  ...
  removeSelectedTracks() {
    const unselectedTracks = this.props.tracks.filter(
   ...
   */
  render() {
    const selectedTracks = this.props.tracks.filter(
      (track) => track.isSelected
    );
    ...
    return (
      <div className="TrackContextMenu-body">
        ...
        <DeselectOption
          ...
          onClick={this.props.deselectAllTracks}
        />
        <RemoveOption
          numTracks={selectedTracks.length}
          onClick={this.removeSelectedTracks}
        />
        <TrackMoreInfo tracks={selectedTracks} />
        <MatplotMenu
          tracks={selectedTracks}
          onApplyMatplot={this.props.onApplyMatplot}
        />
        <DynamicplotMenu
          tracks={selectedTracks}
          onApplyDynamicplot={this.props.onApplyDynamicplot}
        />
        <DynamicHicMenu
          tracks={selectedTracks}
          onApplyDynamicHic={this.props.onApplyDynamicHic}
        />
        ...
        <DynamicBedMenu
          tracks={selectedTracks}
          onApplyDynamicBed={this.props.onApplyDynamicBed}
        />
      </div>
    );
  }
  ...
  return (
    <div className="TrackContextMenu-item">
      <button
        ...
        onClick={() => props.onCircletRequested(track)}
      >
        Circlet view
      </button>{" "}
      <button
        className="btn btn-warning btn-sm btn-tight"
        onClick={() => props.onChordRequested(track)}
      >
        Chord view
      </button>
    </div>
...
function MatplotMenu(props) {
  ...
  return (
    <div className="TrackContextMenu-item">
      <button
        className="btn btn-info btn-sm btn-tight"
        onClick={() => props.onApplyMatplot(props.tracks)}
      >
        Apply matplot
      </button>
    </div>
...
function DynamicplotMenu(props) {
  ...
  return (
    <div className="TrackContextMenu-item">
      <button
        className="btn btn-info btn-sm btn-tight"
        onClick={() => props.onApplyDynamicplot(props.tracks)}
      >
        Dynamic plot
      </button>
    </div>
...
function DynamicHicMenu(props) {
  ...
  return (
    <div className="TrackContextMenu-item">
      <button
        className="btn btn-info btn-sm btn-tight"
        onClick={() => props.onApplyDynamicHic(props.tracks)}
      >
        Dynamic HiC
      </button>
    </div>
...
function DynamicBedMenu(props) {
  ...
  return (
    <div className="TrackContextMenu-item">
      <button
        className="btn btn-info btn-sm btn-tight"
        onClick={() => props.onApplyDynamicBed(props.tracks)}
      >
        Dynamic Bed
      </button>
    </div>
...
function MenuTitle(props) {
  ...
  const text =
    numTracks === 1
      ? props.tracks[0].getDisplayLabel()
      : `${numTracks} tracks selected`;
...
function DeselectOption(props) {
  ...
  return (
    <div
      onClick={props.onClick}
      className="TrackContextMenu-item TrackContextMenu-hoverable-item-warn"
    >
      {/* eslint-disable-next-line jsx-a11y/accessible-emoji */}☐{" "}
