  ...
  render() {
    ...
    const currentTrackList = addedTracks.map((track, index) => (
      <li key={index}>
        ...
        <span
          className="btn btn-link TrackManager-remove-track-button"
          onClick={() => onTrackRemoved(index)}
        >
          ✘
        </span>
      </li>
    ...
    const submenuButtons = SUBMENUS.map((submenu, index) => (
      <button
        key={index}
        className={
          activeSubmenu === submenu ? "btn btn-primary" : "btn btn-success"
        }
    ...
    if (activeSubmenu) {
      ...
      submenuElement = (
        <div className="TrackManager-submenu">
          <Submenu {...this.props} />
        </div>
      );
    }
    ...
    return (
      <div className="TrackManager-parent">
        <div className="TrackManager-sidebar">
          ...
          <h3>Add tracks</h3>
          <div className="btn-group">{submenuButtons}</div>
        </div>
        {submenuElement}
      </div>
    );
