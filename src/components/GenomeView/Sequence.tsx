...
export class Sequence extends React.PureComponent<SequenceProps> {
  ...
  render() {
    ...
    const baseWidth = (xSpan.end - xSpan.start) / sequence.length;
    if (baseWidth < minXwidthPerBase!) {
      return null;
    }

    const sequenceToDraw = isReverseComplement
      ...
      for (const base of sequenceToDraw) {
        rects.push(
          <rect
            ...
            height={height}
            style={{
              fill: BASE_COLORS[base.toUpperCase()] || UNKNOWN_BASE_COLOR,
            }}
          />
        );
        x += baseWidth;
        ...
        if (seqmode) {
          ...
          letters.push(
            <text
              ...
              style={{
                textAnchor: "middle",
                fill: BASE_COLORS[base.toUpperCase()],
