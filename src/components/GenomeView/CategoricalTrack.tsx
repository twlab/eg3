...
const CategoricalTrack: React.FC<TrackProps> = memo(function CategoricalTrack({
  ...
  function getAnnotationElement(placedGroup, y, isLastRow, index, height) {
    return placedGroup.placedFeatures.map((placement, i) => {
      ...
      const color =
        configOptions.current.category &&
        configOptions.current.category[featureName]
          ? configOptions.current.category[`${featureName}`].color
          : "blue";
  ...
  function refGeneClickTooltip(feature: any, pageX, pageY, name, onClose) {
  ...
  function getCacheData() {
    ...
    if (viewData.length > 0) {
      ...
      if (
        ...
        let deDupRefGenesArr = removeDuplicatesWithoutId(refGenesArray);
