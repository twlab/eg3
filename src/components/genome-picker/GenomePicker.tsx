...
export default function GenomePicker() {
    ...
    const genomeList = useMemo(() => Object.entries(treeOfLife).map(([name, data]: [string, any]) => ({
        name,
    ...
    return (
        <div className="max-w-2xl mx-auto pt-4 h-full">
            ...
            <div className={`grid grid-cols-3 gap-4 ${selectedPath !== null ? 'items-center' : ''}`}>
                {(selectedPath === null ? genomeList : genomeList.filter(g => g.name === selectedPath[0])).map((genome) => (
                    <motion.div
                        key={genome.name}
